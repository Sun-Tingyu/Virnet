...
public class GroupInfoCDAO {
    ...
    public Map<String, Object> deleteGroup(String classgroupName){
        ...
        List<Groupmember> gml = mDAO.getListByProperty("classgroupmemberGroupId", classgroupId);
        System.out.println("groupmemberList:"+gml);
        int gmls = gml.size();
        for(int i = 0; i < gmls; i++) {
        	Groupmember temp = gml.get(i);
        	mDAO.delete(temp);
        }
        //1.7删除classgroup表
