...
//		System.out.println("user:"+user);
	...
	public Map<String, Object>expDateARG(String classInfoNDate){//谭睿雄5.5
		Map<String, Object>map = new HashMap<String, Object>();
		List<String> dateValue = new ArrayList<String>();
		DateFormat fmt =new SimpleDateFormat("yyyy-MM-dd");
		List<Date>Pdatelist = new ArrayList<Date>();
		PeriodarrangeDAO pdao = new PeriodarrangeDAO();
		System.out.println("classInfo&&Date:"+classInfoNDate);
		String[] cndl = classInfoNDate.split("&&"); 
		String classInfo = cndl[0];
		String allDate = cndl[1];	
		System.out.println("classInfo:"+classInfo+" allDate:"+allDate);
		if(allDate.equals("undefined")) {
			...
			if(Pdatelist.size() == 0) System.out.println("Error fetch Periodarrange list");
				...
				for(int i = 0;i<Pdatelist.size();i++){
					System.out.println("Pdatelist.get("+i+"):"+Pdatelist.get(i)+" fmt:"+fmt.format(Pdatelist.get(i)));
			...
			String[] date = allDate.split(" - ");
			String[] endDatetime = date[1].split(" ");
			String endDate = endDatetime[0];
			System.out.println("classInfo:"+classInfo+" endDate:"+endDate);
