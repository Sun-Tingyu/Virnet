...
import java.io.DataInputStream;//trx
...
public class MainSystemWebSocketHandler extends TextWebSocketHandler implements WebSocketHandler {
	...
	private static final int resourceServerPort = 8341;//trx
	static int operationServerPort = 8342;//trx
	static int heartBeatServerPort = 8343;//trx
	private static boolean heartBeatLock = true;//trx
	private Date date = new Date();//trx
	static {
		arrangeUsers = new ArrayList<>();
	}

	private static final ArrayList<WebSocketSession> expUsers;
	private static final ArrayList<String> userNames = new ArrayList<>();//trx
	...
	private static ConcurrentHashMap<WebSocketSession, String> MapUserName1 = new ConcurrentHashMap<WebSocketSession, String>();
	...
	private static ConcurrentHashMap<WebSocketSession, String> userMap1 = new ConcurrentHashMap<WebSocketSession, String>();
	// 记录实验中用户和用户连接状态的map，用于监控员监控用户连接状态-trx
	private static ConcurrentHashMap<String, String> studentConnectionMap = new ConcurrentHashMap<String, String>();
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		System.out.println("websocket is established at " + date.toString());
		...
		String expId = (String) session.getAttributes().get("WS_USER_expId");
//		String isreboot = (String) session.getAttributes().get("WS_USER_reboot");

		MapUserName.put(session, userName);
		MapUserName1.put(session, userName);
...
//		System.out.println("handleMessage1:" + message);
		...
		//谭睿雄12.2-2019/06/06:检验实验中用户的连接状态（包括前端与三层、三层与二层两种连接状态）
		else if(jsonString.getString("flag").equals("ping")&&heartBeatLock){
			heartBeatLock = false;
			System.out.println("ping!");
			//与二层建立连接
			try(Socket connectToServer = new Socket(MapEquipmentIp.get(userMap.get(wss)),heartBeatServerPort)){							
				...
				//与二层连接成功后发送消息给前端实验员
				jsonString.put("handshake", "success");
				wss.sendMessage(new TextMessage(jsonString.toString()));
				// 保存连接状态信息以备发给前端监控员
				if(!monitorUsers.contains(wss)){
					String userName = MapUserName.get(wss);
					String userGroup = userMap.get(wss);
					studentConnectionMap.put(userName + "&" + userGroup, "1" );
				}				
			} 
			catch (Exception e) {
				...
				//与二层连接失败后，断开三层与前端的连接
				wss.close();
				System.out.println("websocket is closed for heartBeat's exception!");
			} 
			System.out.println("ping finished!");
			heartBeatLock = true;
		}
		else if(!heartBeatLock){
			System.out.println("heartBeatLock!");
		}
		// 谭睿雄2019/07/24：监控员获取实验员连接情况
		else if(jsonString.getString("flag").equals("pingFromMonitor")){
			System.out.println("pingFromMonitor!");
			String userAndGroup = "";
			
			for(String key : studentConnectionMap.keySet()){
//				if (!monitorUsers.contains(user)){
//					String userName = MapUserName1.get(user);
//					//userNames.add(userName);
//					String userGroup = userMap1.get(user);
//					if(!studentConnectionMap.containsKey(user)){
//						studentConnectionMap.put(user,"0");
//					}
//					String studentConnectionState = studentConnectionMap.get(user);
					userAndGroup = userAndGroup + key + "&"+studentConnectionMap.get(key)+"%%" ;
					studentConnectionMap.put(key, "0");
				
			}
			for(WebSocketSession monitor : monitorUsers){
				JSONObject jsonString1 =new JSONObject();
				jsonString1.put("type", "studentConnection");
				jsonString1.put("userAndGroup", userAndGroup);
				try {
					System.out.println("send to monitor:" + jsonString1.toString());
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}

				monitor.sendMessage(new TextMessage(jsonString1.toString()));
			}
			System.out.println("pingFromMonitor finished!");
		}
		// 判断是否属于监控页面通信.
		...
		// 判断是否属于实验界面
			...
			if(jsonString.getString("experimentStatus").equals("reboot")){
				System.out.println("重启实验桌面");
...
//				System.out.println("usermap==);
				...
				rec.reboot(workgroup, expId, MapEquipment, MapEquipmentIp);
			}
			else if(jsonString.getString("experimentStatus").equals("changeCabinet")){
				System.out.println("更改机柜后，重启实验桌面");
				reconnect rec = new reconnect(); 
 //				System.out.println("usermap==);
				System.out.println("wss=="+wss);
				...
				String workgroup = (String) session.getAttributes().get("WS_USER_WorkGroup");
				String expId = (String) session.getAttributes().get("WS_USER_expId");
				String cabinetIp = (String) session.getAttributes().get("WS_USER_cabinetIP");
				
				rec.changeCabinet(workgroup, expId, cabinetIp , MapEquipment, MapEquipmentIp);
			}
			
			
			// 实验开始初始化资源域
			else if (jsonString.getString("experimentStatus").equals("start")) {
				//String groupid = null;
				try {
				...
				System.out.println("group id === " + groupId + "groupResourceDistribution.get(groupId)==" + groupResourceDistribution.get(groupId) + "MapRealExpStuNum.get(groupId)=="+MapRealExpStuNum.get(groupId));
...
//				else if (groupResourceDistribution.get(groupId) > standardExpNum) { 
					...
					rec.recover(jsonString, wss, userMap, MapEquipmentName, MapEquipmentNum, MapEquipmentPort,
							...
							groupMemberMap, userFacilityConfigureMap, userPcConfigureMap,MapEquipmentIp);
				}
			}
			...
			// 输入的设备命令和输出设备结果处理域
				...
				if (sourceStrArray[Integer.parseInt(jsonString.getString("inputEquipmentNumber"))].equals("PC")) {
					System.out.println("equalsPC!");//trx
			...
			// 聊天信息处理域
				...
				// 监控管理员消息，全员播报
				System.out.println("monitor broadcast!");
				if (monitorUsers.contains(wss)) {
...
//			 }
			...
			if (userNames.contains(MapUserName.get(wss)))
				userNames.remove(MapUserName.get(wss));//trx
			JSONObject jsonString2 = new JSONObject();
			jsonString2.put("type", "studentConnection");
			jsonString2.put("disconnection", MapUserName.get(wss));
			sendToMonitor(wss,jsonString2);
			if (expUsers.contains(wss))
				expUsers.remove(wss);
			if (userMap.containsKey(wss))
				userMap.remove(wss);
			...
			System.out.println("connection is closed at " + date.toString());
	...
	public synchronized void queueingLogic(JSONObject jsonString, WebSocketSession wss) throws IOException {
		...
		for (WebSocketSession successMatch : record) {
			...
			System.out.println("SuccessMatch== "+successMatch+"group=="+group);
			userMap.put(successMatch, group);
			userMap1.put(successMatch, group);
	...
	//将实验中学生的连接情况发送给监控员     *实验阶段*-trx
	public void sendStudentConnection(WebSocketSession wss, JSONObject jsonString) {
...
//	public void reboot(WebSocketSession wss){
//		reconnect rec = new reconnect();
//		rec.recover(jsonString, wss, userMap, MapEquipmentName, MapEquipmentNum, MapEquipmentPort,
//				MapEquipment, MapTopo, MapGroupProblem, groupFacilityConfigureMap, groupPcConfigureMap,
//				expUsers, monitorUsers, MapCommandHistory, MapGroupEndTime, MapTaskOrder, MapUserName,
