function expArgEdit(){
	...
	var button3 = $("<button id = 'delete' class = 'btn button-new' onclick = 'deleteEditARG()'>删除</button>" );//谭睿雄4.1
	var button2 = $("<button id = 'cancel' class = 'btn button-new' onclick = 'cancelEditARG()'>取消</button>" );
	button2.appendTo('#content');
	button3.appendTo('#content');//trx4.1
...
function createModeSelectorARGInEdit(father,classinfo,value){
	//排课模式选择器
	var select = $("<select></select>");
	...
	select.on("change", function(){//谭睿雄5.7
		//更改操作类型  新增/删除
		...
		$("select[id=exp-arrangement]").empty();
		var option_head = $("<option></option>");
		option_head.attr("value", 0);
		option_head.html("请选择排课日期:");
		option_head.appendTo($("select[id=exp-arrangement]"));
		   ...
		   // async: false,
		    success : function(data){
		    	for(var i=0; i<data['date'].length;i++){
		    		...
		    		option.appendTo($("select[id=exp-arrangement]"));
		    	}
		    	$("select[id=exp-arrangement]").select2("val", 0);
		    },
		    error : function(data){
		    	alert("error");
		    }
...
function createExpDateSelectorARG(classinfo, op, father, timespan_id,value){//
	//排课日期选择器
	$("select[id=exp-arrangement]").empty();//谭睿雄5.3
	if(value==undefined){
		var select = $("<select></select>");
		select.attr("id", id);
		select.attr("class","form-control select select-primary select-block mbl");
		...
		select.on("change", function(){
			if(op == 0){
				...
				if(value1 == 0) createMutiSelect_ARGtimelist(td_timespan, 'expTimespan', "first_in");;
	...
	$.ajax({
		url : "expDateARG.action",
	    data : {
			classinfo : classinfo+"&&"+value,//谭睿雄2019.5.4
	    },    
	    type : 'post',      
	    dataType : 'json',
	   // async: false,
...
function add_exp_onButton(classinfo,value){
	var op = $("#modeSelectInEdit").val();
	add_exp(classinfo, op,value);
}
function add_exp(classinfo, op,value){
	tbody = $("#tbody");
	...
	var td_expdate = $("<td class=td_expdate></td>");//谭睿雄2019.5.6
	$("<th class = 'col-lg-4 col-md-4 col-sm-4 col-xs-6'>实验日期</th>").appendTo(tr_expdate);
	createExpDateSelectorARG(classinfo, 0, td_expdate,'expTimespan'+i,value);
...
function createARG(){
	...
	var td_select = $('<td></td>');
	createModeSelectorARGInEdit(td_select,classinfo);
	...
	laydate.render({
	   ...
	   min: 0,
		   ...
		   createExpDateSelectorARG(classinfo, 0, $(".td_expdate"), 'expTimespan1',value);//谭睿雄5.1
	   }
	});
}
//old not used
...
//谭睿雄4.3-删除课程安排
function deleteEditARG(){
	var classinfo = $('select').first().val();
	console.log("classinfo:"+classinfo+" username:"+username);
	var deletetr = $('tr.delARG p:eq(0)').text();
	console.log("deletetr:"+deletetr);
	var info = "id" + "@@" + deletetr+"&&"+username ;
	              ...
	              action: function(){
	                  $.ajax({
	                      ...
	                      success:function(data) {
	                          ...
	                          showContent("exp-arrangement",0);	                          
	                      },
	                      error:function(){//谭睿雄6.4
	                    	  alert("存在实验实例，无法删除！");
	                      }
	                  });
	                  return msg.cancel();	                  
